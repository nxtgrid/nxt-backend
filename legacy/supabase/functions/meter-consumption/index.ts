...
async function getMeterConsumption(req) {
  ...
  // If we have okp1 or okp2, then limit the search to okp
  ...
  try {
    ...
    const countResult = await client.queryObject(countQuery, [fromDate, toDate]);
    ...
    const result = await client.queryObject(dataQuery, [fromDate, toDate, pageLimit, offset]);
