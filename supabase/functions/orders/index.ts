...
async function getTask(req, supabase, user) {
  ...
  const { data, count: total, error } = await supabase
    .from('orders')
    .select('*, grids!inner(name)', { count: 'exact' })
    .eq('meta_is_hidden_from_reporting', false)
    .eq('meta_order_type', 'ENERGY_TOPUP')
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/orders' \
