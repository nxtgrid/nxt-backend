...
| 35 | `grids.timezone` column default | Parameterize/neutralize default: `'Africa/Lagos'` → `'UTC'` | De-Nigeria the OSS baseline (002d decision 12; i18n register #1) | **New inserts only** — existing company grid rows keep stored values. At cutover: optional `ALTER TABLE grids ALTER COLUMN timezone SET DEFAULT 'UTC'` if NXT Grid wants the column default aligned; no row data migration required | confirmed |
| 36 | **Rename enum value — `meter_interaction_type_enum`:** `TOP_UP` → `TOP_UP_KWH` | Rename enum value | Clarify unit (kWh top-up vs other future credit units); OSS baseline uses `TOP_UP_KWH` in init migration. Legacy/company production still emits and stores `TOP_UP`. ADR-012 decision 1 flip-atomic example | **Flip-atomic** (ADR-012): `ALTER TYPE public.meter_interaction_type_enum RENAME VALUE 'TOP_UP' TO 'TOP_UP_KWH'` in lockstep with new backend/frontend deploy (all writers of `meter_interactions.meter_interaction_type` and `meter_command_batches.task_type`). See Programmability adjustments §11 | confirmed |
...
### §11 — Motivated by register #36 (`meter_interaction_type_enum` value rename)

| Object | Change | Rationale | Cutover / code impact |
|--------|--------|-----------|------------------------|
| `meter_interaction_type_enum` | Rename value `TOP_UP` → `TOP_UP_KWH` in init migration | Unit-explicit name; same semantic (kWh credit top-up) | **Flip-atomic:** `ALTER TYPE … RENAME VALUE 'TOP_UP' TO 'TOP_UP_KWH'` with new backend/frontend; existing rows and `meter_command_batches.task_type` values update in place via the enum rename |
...
| #36 | 1 | meter_interaction_type_enum `TOP_UP` → `TOP_UP_KWH` |
